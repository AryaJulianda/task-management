        ...
        <div class="card-body p-4">
          ...
          <form method="post" action="{{ route('tasks.update', [$project, $task]) }}">
            ...
            <div class="mb-3">
              ...
              <input class="form-control" id="title" name="title" type="text" value="{{ old('title', $task->title) }}" required>
            </div>
            <div class="mb-3">
                ...
                @forelse ($users as $user)
                  <div class="col-md-4">
                    <div class="form-check">
                      <input class="form-check-input" type="checkbox" name="assignees[]" id="assignee-{{ $user->id }}" value="{{ $user->id }}" @checked(in_array($user->id, old('assignees', $task->assignees->pluck('id')->all())))>
            ...
            <div class="d-flex gap-2 mt-4">
              <button class="btn btn-dark" type="submit">Update task</button>
              <a class="btn btn-light" href="{{ route('tasks.show', [$project, $task]) }}">Cancel</a>
            </div>
          </form>
        </div>
      </div>
